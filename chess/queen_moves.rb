...
def bishop_moves(str)
	arr = convert_chess_notation_to_array(str)
	result_ar=[]
	i = arr[0]
  j = arr[1]
		# up and right
...
def queen_moves(str)
  ...
  possible_queen_moves.push(rook_moves(str), bishop_moves(str))
  possible_queen_moves.map do |pqm|
  	convert_array_to_chess_notation(pqm)
  end
	puts "\n"
	puts "The queen at #{str} can move to:"
	p possible_queen_moves.flatten
end

def rook_moves(str)
	rook_position = convert_chess_notation_to_array(str)
  possible_rook_moves = []
  0.upto(7) do |i|
  ...
  x = possible_rook_moves.map do |result|
		convert_array_to_chess_notation(result)
	end
	# p result_ar
	p "The rook on #{str} can move to:"
	p x
end
...
def test_many(num)
	num.times do
		queen_moves(random_chess_board_square)
  end
end
