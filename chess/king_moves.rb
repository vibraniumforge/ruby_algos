...
def king_moves(str)
	king_position = convert_chess_notation_to_array(str)
	possible_king_moves = []
	(king_position[0]-1).upto(king_position[0]+1) do |i|
    (king_position[1]-1).upto(king_position[1]+1) do |j|
      if i.between?(0,7) && j.between?(0,7) && [i, j] != king_position
        possible_king_moves << [i, j]
      end
    end
  end
  # print "possible_rook_moves= #{possible_rook_moves}"
  x = possible_king_moves.map do |result|
		convert_array_to_chess_notation(result)
	end
	p "The king on #{str} can move to:"
	p x
end
...
def test_many(num)
	num.times do
		king_moves(random_chess_board_square)
  end
end
