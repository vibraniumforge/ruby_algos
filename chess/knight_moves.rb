...
def knight_moves(str)
	ar = convert_chess_notation_to_array(str)
  ...
  possible_knight_moves << [ar[0]-2, ar[1]+1] if (ar[0]-2).between?(0,7) && (ar[1]+1).between?(0,7)
	possible_knight_moves << [ar[0]-1, ar[1]+2] if( ar[0]-1).between?(0,7) && (ar[1]+2).between?(0,7)
  x = possible_knight_moves.map do |pkm|
		convert_array_to_chess_notation(pkm)
	end
	puts "\n"
	puts "The knight at #{str} can move to:"
	p x
end
...
def test_many(num)
	num.times do
		knight_moves(random_chess_board_square)
  end
end
